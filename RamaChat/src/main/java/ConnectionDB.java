...
public class ConnectionDB {
	...
	public boolean connectar() {
		...
		try {
			this.conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/xat","root","");
	...
	public boolean searchUserBySession(User u) {
		...
		String query = "SELECT * FROM persona WHERE mail= "+email+" AND session="+u.getSession()+";";
		try {
			ResultSet rs = st.executeQuery(query);
			while (rs.next()) {
				u.setUser(rs.getString("user"));
				u.setPass(rs.getString("pass"));
				u.setCodeCountry(rs.getString("country"));
			String fecha = String.valueOf( rs.getDate("last_log"));

				u.setLastLog(LocalDate.parse(fecha));
			}
	...
	public String getFriends(String email) {
		...
		try {
			String query = "SELECT mail2 FROM amistad WHERE mail1='" + email +"';";
			ResultSet rs = st.executeQuery(query);
			Class.forName("org.json.JSONObject");
			
			while (rs.next()) {
				JSONObject json = new JSONObject();
				String mail = rs.getString("mail2");
				json.put("gmail",mail);
				ja.put(json);
			}
			jsonString = ja.toString();
	...
	public boolean buscarcorreoycontra(String mail,String pass) {
		String query = "SELECT * FROM persona WHERE mail='"+mail+"' AND pass =" +pass +";";
