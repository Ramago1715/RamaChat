...
public class User {
	...
	public boolean login() {
		...
		if (this.getUser()!=null) {
			...
			for (int i = 0; i<9;i++) {
				sessiont+=random.nextInt(10);
			}
			String sessionS = String.valueOf(sessiont);
			setSession(sessionS);
			String query="UPDATE persona SET last_log='"+getLastLog()+"',session= "+getSession()+" WHERE mail='"+mail+"';";
	...
	public String getFriends() {
		...
		ConnectionDB conDB = new ConnectionDB();
		conDB.connectar();
		String resposta = conDB.getFriends(getMail());
		conDB.close();
		return resposta;
	}
